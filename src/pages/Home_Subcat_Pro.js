...
function Home_Subcat_Pro() {
    ...
    useEffect(()=>{fetch_procducts();},[]);

    const fetch_procducts = async () => {
        const response = await axios.post('http://127.0.0.1:5001/subcat_products_home/' + window.location.pathname.split('/Home_Subcat_Pro/')[1]);
    ...
    useEffect(() => { filterproducts(); }, [min_price,max_price,star_rating]);
                            ...
                            <div className="col-9 ">
                                <div className="row border me-2 my-2 bg-white">
                                    <div className="col ">
                                        ...
                                        <h6 className='ms-1'>Monitors<span className="txt-frmt20 ms-1">(Showing 1 – {count} products of {count} products)</span></h6>
                                    </div>
                                </div>
...
export default Home_Subcat_Pro;
